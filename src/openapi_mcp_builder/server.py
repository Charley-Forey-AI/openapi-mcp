...
@mcp.tool(
    ...
    description=(
        ...
        "with `include_tags` / `include_paths` / `include_operations` is applied "
        "locally to reduce the file before upload when the raw op count would exceed "
        "the platform limit (so create works without a separate reupload). Set "
        "CREATE_AUTO_TRIM_ON_TOOL_FILTER=0 to disable. "
